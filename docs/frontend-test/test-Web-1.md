...
* 报告 ID：Hyacinth-Web-1
* 测试阶段：第一轮测试
* 测试人员：L
* 测试日期：2025 年 05 月 23 日
* 被测软件版本：v1.0.0
* 测试目标：
  * 验证产品介绍页的显示和基本导航功能。
...
使用之前注册的 17 个账户进行测试，同用户名检测均通过（下图为第 1 个用例）：
...
对于每组用例：若项目 1、项目 2 未通过，则不再检测项目 3、项目 4

检测结果如下，其中 √ 表示达到预期结果：

| 用例编号 | 用户名 + 正确密码 | 邮箱 + 正确密码 | 项目 3 | 项目 4 |
| :------: | :---------------: | :-------------: | :---: | :---: |
|    1     |         √         |        √        |   √   |   √   |
|    2     |     无法登录      |        √        |       |       |
|    3     |     无法登录      |        √        |       |       |
